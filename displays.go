...
func (df *DataFrame) DisplayBrowser() error {
    ...
    <html>
      ...
      <body>
        <div id="app" class="h-screen pt-12" style="text-align: center;">
          <button class="btn btn-sm fixed top-2 left-2 z-50" @click="openInNewTab">Open in Browser</button>
          <div class="dropdown dropdown-end  dropdown-hover fixed top-10 right-2 z-50">
            <div tabindex="0" role="button" class="btn btn-sm fixed top-2 right-2 z-50">Export</div>
            <ul tabindex="0" role="button" class="dropdown-content menu bg-base-200 rounded-box w-56">
            <li>
                <details >
                <summary>To File</summary>
                <ul>
                <li><a @click="exportCSV()">CSV</a></li>
                <li><a @click="exportJSON()">JSON</a></li>
                <li><a @click="exportNDJSON()">NDJSON</a></li>
                </ul>
                </details>
            </li>
            <!-- <li>
                <details >
                    <summary>To Chart</summary>
                    <ul>
                    <li><a>Bar Chart</a></li>
                    <li><a>Stacked Bar Chart</a></li>
                    <li><a>Stacked Percentage Chart</a></li>
                    <li><a>Column Chart</a></li>
                    <li><a>Line Chart</a></li>
                    <li><a>Pie Chart</a></li>
                    <li><a>Scatter Plot</a></li>
                    </ul>
                </details>
                </li> -->
            </ul>            
        ...
        <div class="fixed top-2 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
            ...
            <div class="join pointer-events-auto">
                ...
                <span v-if="pages <= 6" v-for="page in page_list" class="join">
                    <button v-if="current_page === page" class="btn btn-sm btn-active no-animation join-item rounded-none"><a href="#!">[[ current_page ]]</a></button>
                    <button v-else class="btn btn-sm join-item rounded-none" @click="pagefunc(page)"><a href="#!">[[ page ]]</a></button>
                </span>
						...
						<tr>
                            ...
                            <th v-for="col in cols"  class="sticky top-12 z-40 bg-base-100 p-2">
                                <div class="dropdown dropdown-hover">
                                    ...
                                    <ul tabindex="0" class="dropdown-content menu bg-base-200 rounded-box z-[1]  shadow">
                                        <li>
                                            <details closed>
          ...
          createApp({
            ...
            methods: {
              ...
              exportNDJSON() {
                ...
                // );
                const df = ReadJSON(this.data);
                df.ToNDJSONFile('dataframe.ndjson');
              },
              sortColumnAsc(col) {
