...
func referencedCols(e ColumnExpr, acc map[string]struct{}) map[string]struct{} {
	...
	default:
		var x ColumnExpr
		if len(e.Expr) > 0 {
			_ = json.Unmarshal(e.Expr, &x)
			referencedCols(x, acc)
		}
		if len(e.Left) > 0 {
			_ = json.Unmarshal(e.Left, &x)
			referencedCols(x, acc)
		}
		if len(e.Right) > 0 {
			_ = json.Unmarshal(e.Right, &x)
			referencedCols(x, acc)
		}
		if len(e.Cond) > 0 {
			_ = json.Unmarshal(e.Cond, &x)
			referencedCols(x, acc)
		}
		if len(e.True) > 0 {
			_ = json.Unmarshal(e.True, &x)
			referencedCols(x, acc)
		}
		if len(e.False) > 0 {
			_ = json.Unmarshal(e.False, &x)
			referencedCols(x, acc)
		}
		if len(e.Cols) > 0 {
			var xs []ColumnExpr
...
// canonicalKey builds a deterministic string representation for arbitrary
// values (scalars, slices, maps) so it can safely be used as a map key.
func canonicalKey(v interface{}) string {
	...
	case map[string]interface{}:
		// deterministic iteration by sorting keys
		...
		return b.String()
	case map[interface{}]interface{}:
		// convert keys to string then recurse
		m := convertMapKeysToString(x)
...
func (df *DataFrame) DropNA() *DataFrame {
	...
	for g := 0; g < w; g++ {
		...
		if start >= df.Rows+counts[g] {
			break
		} // defensive
		if end > df.Rows+counts[g] {
			end = df.Rows + counts[g]
		}
		base := offsets[g]
		mask := masks[g]
		wg.Add(1)
...
func (left *DataFrame) Join(right *DataFrame, leftOn, rightOn, joinType string) *DataFrame {
	...
	// Build indices
	leftIndex := make(map[string][]int, left.Rows)
	rightIndex := make(map[string][]int, right.Rows)
	var wg sync.WaitGroup
	wg.Add(2)
	go func() {
		defer wg.Done()
